...
class Linear_model:
    ...
    def __init__(self) -> None:
        self.fc1 = Linear_layer(784, 128, init_mode="HeInit")
        self.fc2 = Linear_layer(128, 64, init_mode="HeInit")
        self.fc3 = Linear_layer(64, 10, init_mode="HeInit")

        self.relu = ReLU()

    def forward(self, x):
        x = flatten_mnist(x)
        A = self.relu.forward(self.fc1.forward(x))
        A = self.relu.forward(self.fc2.forward(A))
    ...
    def backward(self, dS):
        ...
        dA = self.fc2.backward(self.relu.backward(dA))
        dX = self.fc1.backward(self.relu.backward(dA))
